...
exports.all_places_in_the_city = async(city_id)=>{
    return await db.query(`SELECT * FROM places INNER JOIN cities ON places.cities_id = cities.id WHERE cities.id = ${city_id};`); 
}
...
exports.get_a_city_byName = async (city_name)=>{
    return await db.query(`SELECT id FROM cities WHERE name = "${city_name}";`);
}
